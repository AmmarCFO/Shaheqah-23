...
const App_ar: React.FC<{ onToggleLanguage: () => void }> = ({ onToggleLanguage }) => {
  ...
  const translateScenarioName = (id: string) => {
      switch(id) {
          case 'combined': return 'شاهقة ٢٣ (كامل المحفظة العقارية)';
          case 'str': return 'فئة قصير الأجل STR (٤١ وحدة)';
          case 'ltr': return 'فئة طويل الأجل LTR (٢٣ وحدة)';
          default: return id;
      }
  };
  ...
  const translateScenarioDesc = (id: string) => {
    switch(id) {
        case 'combined': return 'تحليل شامل لمحفظة مميزة تضم ٦٤ شقة غرفتين وصالة لربط النماذج الهجينة والمشتركة بالرياض. يجمع بين ٤١ شقة قصير الأجل و ٢٣ شقة طويل الأجل لتحقيق عوائد سنوية قصوى مع حماية تامة للهامش المالي.';
        case 'str': return 'المحفظة السكنية قصيرة الأجل (STR) وتضم ٤١ وحدة للتأجير اليومي والشهري الفاخر. تركز بشكل خالص على اقتناص أعلى هوامش تشغيلية خلال مواسم الفعاليات والترفيه مع استقرار تشغيلي متوازن.';
        case 'ltr': return 'دراسة محفظة عقود طويل الأجل (LTR) وتضم ٢٣ شقة فخمة مؤجرة بعقود سنوية مستقرة لتوفير ريع مالي متوقع وتقليل التكاليف التشغيلية ومخاطر الشواغر.';
        default: return '';
    }
  };

  const translateUnitMixLabel = (name: string) => {
     if (name.includes('Short-Term') || name.includes('STR')) return 'شقة غرفتين وصالة (تأجير قصير الأجل) - ٤١ شقة';
     if (name.includes('Long-Term') || name.includes('LTR')) return 'شقة غرفتين وصالة (تأجير طويل الأجل) - ٢٣ شقة';
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pb-16">
        ...
        <FadeInUp>
          <div className="text-center pt-8 pb-8 sm:pt-14 sm:pb-8">
            ...
            >
                <span className="text-[10px] sm:text-xs font-bold tracking-[0.22em] uppercase text-purple-700">مشروع شاهقة ٢٣ - لوحة قياس الجدوى العقارية</span>
            </motion.div>
            <h1 className="text-4xl sm:text-7xl font-black text-[#1D1D1F] tracking-tighter mb-4 leading-none">
              شاهقة ٢٣<span className="text-[#4A2C5A]">.</span>
            </h1>
            <p className="text-sm sm:text-2xl text-gray-500 max-w-3xl mx-auto font-medium leading-relaxed tracking-tight px-4 mb-8">
                تحليل العقار المميز بالرياض في إطار <span className="text-[#4A2C5A] font-bold">النماذج الهجينة المشتركة</span>. ٦٤ وحدة سكنية متكاملة تشمل شقق غرفتين وصالة بنوعيها قصير وطويل الأجل.
            </p>
          </div>
        </FadeInUp>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 sm:mb-12 text-right">
          <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
            ...
            <span className="text-xl sm:text-3xl font-extrabold text-[#4A2C5A] tracking-tight">٦٤ وحدة سكنية</span>
            <span className="text-xs text-gray-500 block mt-1">كافة التصاميم فئة غرفتين وصالة</span>
          </motion.div>
          <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">فئة قصير الأجل (STR)</span>
            <span className="text-xl sm:text-3xl font-extrabold text-amber-600 tracking-tight">٤١ وحدة سكنية</span>
            <span className="text-xs text-gray-500 block mt-1">شقق غرفتين وصالة - تشغيل يومي</span>
          </motion.div>
                    <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">فئة طويل الأجل (LTR)</span>
            <span className="text-xl sm:text-3xl font-extrabold text-blue-600 tracking-tight">٢٣ وحدة سكنية</span>
            <span className="text-xs text-gray-500 block mt-1">عقود سكنية سنوية مستقرة</span>
          </motion.div>
          <motion.div whileHover={{ y: -3 }} className="bg-white border border-gray-200 rounded-2xl p-4 sm:p-6 shadow-sm">
        ...
        <div className="mb-10 text-right font-cairo">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-6 gap-2">
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1">اختر أحد نماذج التشغيل الثلاثة الرئيسية لتشغيل كامل محفظة شاهقة ٢٣ المشتركة لمعاينة الأثر المالي والفني فورياً.</p>
            </div>
          </div>
            ...
            >
              <div className="w-full">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs font-mono font-bold text-gray-400">٢٣ وحدة</span>
                </div>
                <h4 className="font-bold text-gray-900 text-base mb-1 font-cairo">عقود سكنية طويلة الأجل</h4>
                <p className="text-xs text-gray-500 leading-relaxed font-cairo">عقود سنوية مستقرة للشركات والتنفيذيين بالرياض، تلغي فترات الشاغر وتوفر موازنة ريعية آمنة.</p>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between w-full">
                ...
                <span className="text-sm font-extrabold text-[#AA7C11]">٢,٠٨٠,٢١٢ ر.س</span>
              </div>
            </button>
            ...
            >
              <div className="w-full">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs font-mono font-bold text-gray-400">٤١ وحدة</span>
                </div>
                <h4 className="font-bold text-gray-900 text-base mb-1 font-cairo">تأجير يومي قصير الأجل</h4>
                <p className="text-xs text-gray-500 leading-relaxed font-cairo">تسعير مرن يقتنص طفرات المعارض والترفيه ويرفع عوائد التشغيل الفندقي لأقصى هوامش ممكنة.</p>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between w-full">
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider">الإيرادات بمستوى الأساس</span>
                <span className="text-sm font-extrabold text-[#D4AF37]">١٠,١٧٢,١٠٠ ر.س</span>
              </div>
            </button>
            ...
            >
              <div className="w-full">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className="text-xs font-mono font-bold text-gray-400">٦٤ وحدة كاملة</span>
                </div>
                <h4 className="font-bold text-gray-900 text-base mb-1 font-cairo">النموذج الهجين المتكامل</h4>
                <p className="text-xs text-gray-500 leading-relaxed font-cairo">الخلطة الاستراتيجية: ٤١ وحدة يومي للمواسم الدافئة عوائدياً، و٢٣ وحدة عقود سنوية كركيزة حماية تشغيلية.</p>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between w-full">
                <span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider">الإيرادات بمستوى الأساس</span>
                <span className="text-sm font-extrabold text-[#4A2C5A]">١٢,٢٥٢,٣١٢ ر.س</span>
              </div>
            </button>
          </div>
        </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                         ...
                         <div className="flex items-center gap-3 text-right w-full md:w-auto">
                             ...
                             <span className="text-xs sm:text-sm font-bold text-white font-cairo">
                                 {activeModel === 'ltr' 
                                   ? "محفظة عقود طويل الأجل (٢٣ وحدة سكنية)" 
                                   : activeModel === 'str' 
                                     ? "محفظة الإيجار قصير الأجل (٤١ وحدة سكنية)" 
                                     : "كامل محفظة شاهقة ٢٣ المشتركة (٦٤ وحدة سكنية)"}
                             </span>
                         </div>
